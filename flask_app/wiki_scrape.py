...
def get_wiki_str(year: str, specific_word: str) -> str:
    month_list = [
        "januari",
        "februari",
        "maart",
        ...
        for ul_tag in soup.findAll("ul"):
            for li_tag in ul_tag.find_all("li"):
                ...
                if a_tag and re.search("wiki/\d{1,2}", a_tag["href"]):
                    ...
                    n_month_new = next(
                        (n for n, month in enumerate(month_list) if month in date_tag),
                        -1,
                    )
    ...
    if len(df_events) > 0:
        wiki_str = (
            "In dit jaar waren er de volgende gebeurtenissen"
            + "die te maken hebben met <mark>"
            + specific_word
            + "</mark><br>"
        )
